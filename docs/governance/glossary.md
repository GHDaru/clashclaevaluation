...
| `[DOMAIN]` **YellowCard** | Cartão Amarelo | Penalidade por 1 ataque faltante em um WarDay. | ADR 001 |
| `[DOMAIN]` **RedCard** | Cartão Vermelho | Penalidade resultante da conversão de 4 YellowCards. | ADR 001 |
| `[DOMAIN]` **BlackCard** | Cartão Preto | Penalidade máxima — conversão de 4 RedCards. Candidato a expulsão. | ADR 001 |
| `[DOMAIN]` **War** | Guerra | Ciclo de batalha de quinta a domingo (4 WarDays). | ADR 001 |
| `[DOMAIN]` **WarDay** | Dia de Guerra | Um dos 4 dias: Thursday, Friday, Saturday, Sunday. | ADR 001 |
| `[DOMAIN]` **Attack** | Ataque | Uma batalha de guerra (1 dos 4 decks por WarDay). | ADR 001 |
| `[DOMAIN]` **ChampionRace** | Corrida dos Campeões | Janela de recência de 4 semanas com peso máximo para avaliação. | ADR 001 |
| `[DOMAIN]` **EarlyVictory** | Vitória Antecipada | Clã cruzou a finish line em 1º lugar antes de Sunday. | ADR 001 |
| `[DOMAIN]` **Relaxation** | Relaxamento | Suspensão de emissão de cartões nos WarDays restantes após EarlyVictory. | ADR 001 |
| `[DOMAIN]` **Evaluation** | Avaliação | Processo pós-War de atribuir cartões. Executado pelo EvaluationService. | ADR 001 |
| `[DOMAIN]` **CardConversion** | Conversão de Cartões | Regra: 4 YellowCards → 1 RedCard; 4 RedCards → 1 BlackCard. | ADR 001 |
| `[DOMAIN]` **PointThreshold** | Limiar de Pontuação | Mínimo de pontos por War: < 1600 = +1 YellowCard; < mínimo do clã = +1 RedCard. | ADR 001 |
